...
function toolsTemplate(tools = []) {
  ...
  const cards = tools.map((tool) => {
    const signature = tool.signature ? `<pre class="tool-signature"><code>${esc(tool.signature)}</code></pre>` : '';
    const example = tool.example && tool.example !== tool.signature
      ? `<div class="tool-example-label">Пример</div><pre class="tool-example"><code>${esc(tool.example)}</code></pre>`
      : '';
    return `<article class="tool-card">
      <div class="tool-card-head"><code>${esc(tool.name)}</code><span>${esc(tool.kind || 'Инструмент')}</span></div>
      ${signature}<p>${esc(tool.description)}</p>${example}
    </article>`;
  }).join('');
  return `<details class="tool-reference">
    <summary><span>🧰 Забыл функцию или конструкцию?</span><small>Открыть справочник · ${tools.length}</small></summary>
    <p class="tool-reference-intro">Здесь только уже изученные инструменты, нужные для этой задачи. Посмотри форму вызова и вернись к своей попытке.</p>
    <div class="tool-grid">${cards}</div>
  </details>`;
}

function progressiveHintsTemplate(hints = []) {
  const labels = ['направление', 'нужный инструмент', 'алгоритм', 'почти решение'];
  return hints.filter(Boolean).map((hint, index) => {
    const locked = index > 0;
    const requiresAttempt = index === hints.length - 1 && hints.length > 1;
    const label = labels[index] || `шаг ${index + 1}`;
    return `<details class="hint ${locked ? 'locked' : ''}" data-hint-step="${index}" ${requiresAttempt ? 'data-requires-attempt="true"' : ''}>
      <summary><span>Подсказка ${index + 1} · ${label}</span><small>${locked ? '🔒 Сначала открой предыдущую' : 'Открыть'}</small></summary>
      <p>${esc(hint)}</p>
    </details>`;
  }).join('');
}

function unlockHintSequence(card) {
  if (!card) return;
  const hints = [...card.querySelectorAll('[data-hint-step]')];
  hints.forEach((hint, index) => {
    if (index === 0) {
      hint.classList.remove('locked');
      hint.querySelector('summary small').textContent = hint.open ? 'Открыто' : 'Открыть';
      return;
    }
    const previousWasRead = hints[index - 1].open;
    const attemptAllowed = hint.dataset.requiresAttempt !== 'true' || card.dataset.attempted === 'true';
    if (previousWasRead && attemptAllowed) {
      hint.classList.remove('locked');
      hint.querySelector('summary small').textContent = hint.open ? 'Открыто' : 'Открыть';
    }
  });
...
function bindHintControls(scope) {
  scope.querySelectorAll('[data-hint-step]').forEach((hint) => {
    hint.querySelector('summary')?.addEventListener('click', (event) => {
      if (hint.classList.contains('locked')) event.preventDefault();
    });
    hint.addEventListener('toggle', () => {
      if (hint.open) {
        const card = hint.closest('[data-question]');
        if (card && hint.dataset.counted !== 'true') {
          card.dataset.hintsUsed = String(Number(card.dataset.hintsUsed || 0) + 1);
          hint.dataset.counted = 'true';
        }
        unlockHintSequence(card);
      } else if (!hint.classList.contains('locked')) {
        hint.querySelector('summary small').textContent = 'Открыть снова';
      }
    });
  });
}
...
function isModuleCollapsed(module) {
  if (module.completed !== module.total || !module.exam.passed) return false;
...
function renderDashboard(data) {
  ...
  const nextExam = data.next_exam;
  const heroAction = next
    ? `<a class="button light" href="#/lesson/${next.id}">Продолжить <span>→</span></a>`
    : nextExam
      ? `<a class="button light" href="#/exam/${nextExam.module_id}">Сдать экзамен <span>→</span></a>`
      : '<a class="button light" href="#/practice">Закрепить знания <span>→</span></a>';
  const heroTitle = next
    ? `Следующий шаг: ${esc(next.title)}`
    : nextExam
      ? `Следующий шаг: экзамен «${esc(nextExam.module_title)}»`
      : 'Ты прошёл весь путь!';
  const heroLead = next
    ? 'Небольшие уроки, практика и честная обратная связь — чтобы Python действительно запоминался.'
    : nextExam
      ? 'Уроки раздела готовы. Закрепи их в смешанной контрольной — обязательный код проверяется на новом сценарии.'
      : 'Все уроки и экзамены пройдены. Продолжай практиковаться, чтобы знания стали автоматическими.';
...
function renderModule(module) {
  ...
  const header = module.completed === module.total && exam.passed
...
function questionTemplate(question, number, stageLabel = '') {
  ...
  const guide = question.guide
    ...
    : '';
  // В вопросах на узнавание справочник сам раскрывает ответ; он нужен именно при написании кода.
  const toolHelp = question.kind === 'code' ? toolsTemplate(question.tool_help) : '';
  let field = '';
  if (question.kind === 'choice') {
    ...
    field = `<textarea class="code-editor" data-answer-q="${question.id}" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false">${esc(question.starter)}</textarea>
      ...
      <div class="progressive-hints">${progressiveHintsTemplate(question.hints || [])}</div>`;
  }
  return `<article class="question-card card" data-question="${question.id}">${head}${guide}${toolHelp}${field}<div class="inline-result" id="result-${question.id}"></div></article>`;
}
...
function getAnswers(scope, questions) {
  return questions.map((question) => {
    ...
    const card = scope.querySelector(`[data-question="${question.id}"]`);
    return { question_id: question.id, answer, hints_used: Number(card?.dataset.hintsUsed || 0) };
  });
}

function bindQuestionControls(scope) {
  bindHintControls(scope);
...
function showInline(questionId, correct, message, checks = [], output = null) {
  ...
  const details = failedCount ? ` <small>(нужно исправить сценариев: ${failedCount})</small>` : '';
...
function executionPanel(executions, title = 'Код в действии') {
  ...
  const labels = { stdout: 'вывод', call: 'поведение функции', value: 'значение', ast: 'форма кода', source: 'изучаемая конструкция' };
  const cards = executions.map((item) => `<article class="execution-card"><strong>${item.correct ? '✓ Результат прошёл проверку' : '🖥️ Результат этого запуска'}</strong>${item.source ? `<pre class="execution-source">${esc(item.source)}</pre>` : ''}${item.checks?.length ? `<div class="execution-checks">${item.checks.map((check) => `<p><b>${check.passed ? '✓' : '↺'}</b> ${esc(check.scenario ? `${check.scenario}: ` : '')}${esc(labels[check.kind] || 'контрольный сценарий')} — ${check.passed ? 'пройдено' : 'нужно исправить'}</p>`).join('')}</div>` : ''}<span>Вывод в консоли</span><pre class="code-output">${esc(item.output || '— программа ничего не вывела —')}</pre></article>`).join('');
...
function submissionResult(result, retryText = 'Попробовать ещё раз', answers = []) {
  const items = result.results.map((item) => `<div class="result-item ${item.correct ? 'ok' : 'no'}"><b>${item.correct ? '✓ Верно' : '↺ Повтори'}</b> ${esc(item.message || item.explanation || '')}</div>`).join('');
...
async function renderLesson(id) {
  ...
  const theoryCards = lesson.theory.map((card, index) => {
    const exampleLabel = card.language === 'text'
      ? 'Короткий план'
      : card.language && card.language !== 'python'
        ? `Разберём формат · ${esc(card.language)}`
        : 'Разберём пример';
    return `<article class="theory-card card">
      <p class="theory-step">Шаг ${index + 1} из ${lesson.theory.length}</p><h2>${esc(card.title)}</h2><p>${esc(card.text)}</p>
      <p class="example-label">${exampleLabel}</p><pre class="code-example"><code>${esc(card.example)}</code></pre>${card.output !== undefined ? `<div class="example-output"><strong>Что увидим после запуска</strong><pre class="code-output">${esc(card.output || '— программа ничего не вывела —')}</pre></div>` : ''}${card.tip ? `<div class="tip">${esc(card.tip)}</div>` : ''}
    </article>`;
  }).join('');
