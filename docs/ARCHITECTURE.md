...
SQLite хранит XP, стрик, результаты уроков, экзаменов и историю попыток. `db.py` рассчитывает, какие задания нужно повторить, а API определяет доступность следующего урока по предыдущему завершённому. Личная база `python_path.db` остаётся локальной; `tests/conftest.py` подменяет её отдельной временной SQLite-базой для каждого теста, поэтому тесты не могут очистить учебный прогресс.

Таблица `project_progress` хранит завершённые мини-проекты. Первый проект доступен сразу, остальные требуют и завершения предыдущего проекта, и соответствующего числа пройденных уроков.

### Проверка кода

Все учебные задания, проекты и свободная песочница запускают код через один и тот же изолированный раннер. Он поддерживает базовые чистые функции Python, `input()` с заранее переданными строками и перехват консольного вывода; импорты, доступ к файлам и системные операции остаются запрещёнными.
